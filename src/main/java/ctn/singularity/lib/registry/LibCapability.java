...
public final class LibCapability {
  ...
  public static void registrarItem(RegisterCapabilitiesEvent event) {
    for (Item item : BuiltInRegistries.ITEM) {
      if (item instanceof IRandomDamage capability) {
        event.registerItem(LibCapabilitys.RANDOM_DAMAGE_ITEM, (stack, ctx) -> capability, item);
      }

      if (item instanceof IInvincibleTickItem capability) {
        event.registerItem(LibCapabilitys.InvincibleTick.INVINCIBLE_TICK_ITEM, (stack, ctx) -> capability, item);
      }

      if (item instanceof ILcDamageTypeItem capability) {
        event.registerItem(LibCapabilitys.LcDamageType.LC_DAMAGE_TYPE_ITEM, (stack, ctx) -> capability, item);
      }

      if (item instanceof IUsageReqItem capability) {
        event.registerItem(LibCapabilitys.USAGE_REQ_ITEM, (stack, ctx) -> capability, item);
      }
    }
  }
  ...
  public static void registrarEntity(RegisterCapabilitiesEvent event) {
    for (EntityType<?> entityType : BuiltInRegistries.ENTITY_TYPE) {
      event.registerEntity(
        LibCapabilitys.InvincibleTick.INVINCIBLE_TICK_ENTITY, entityType, (entity, ctx) ->
          entity instanceof IInvincibleTickEntity capability ? capability : null);

      event.registerEntity(
        LibCapabilitys.LcDamageType.LC_DAMAGE_TYPE_ENTITY, entityType, (entity, ctx) ->
